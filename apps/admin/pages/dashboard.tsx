...
const DashboardRSV: React.FC = () => {
  ...
  // Usar hook para carregar dados reais
  const { stats, recentBookings, loading: dashboardLoading, error: dashboardError } = useDashboard({
    autoRefresh: true,
  ...
  const quickActions = [
    ...
    {
      title: 'Leilões de Hospedagem',
      description: 'Participar de leilões e economizar',
      icon: <Gift className="w-6 h-6" />,
      action: () => router.push('/leiloes'),
      color: 'bg-red-500 hover:bg-red-600'
    },
    {
      title: 'Viagens em Grupo',
      description: 'Planejar viagens com amigos',
      icon: <Users className="w-6 h-6" />,
      action: () => router.push('/group-travel'),
                ...
                <div className="bg-white rounded-lg shadow p-6">
                  <div className="flex items-center">
                    ...
                    <div className="ml-4">
                      ...
                      <p className="text-2xl font-bold text-gray-900">R$ {stats.monthlyRevenue.toLocaleString('pt-BR', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</p>
                    </div>
                  </div>
                  <div className="mt-4 flex items-center text-sm text-green-600">
                    <TrendingUp className="w-4 h-4 mr-1" />
