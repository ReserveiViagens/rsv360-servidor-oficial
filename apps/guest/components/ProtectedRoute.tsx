...
function ProtectedRouteContent({
  ...
  // Usar useContext diretamente - retorna undefined se não estiver disponível
  ...
  // Durante SSR ou antes de montar, mostrar fallback
  if (!mounted || typeof window === 'undefined') {
    return <>{fallback}</>;
  }

  // Se o contexto não estiver disponível, mostrar fallback
  if (authContext === undefined) {
    return <>{fallback}</>;
  }

  const { isAuthenticated, isLoading, hasPermission } = authContext;
