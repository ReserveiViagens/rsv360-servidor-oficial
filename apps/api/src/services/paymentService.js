...
async function processPayment(paymentData) {
  try {
    ...
    if (splits && splits.length > 0) {
      // Validar estrutura de cada split
      for (const split of splits) {
        if (!split.split_type || !['percentage', 'fixed_amount'].includes(split.split_type)) {
          throw new Error("Cada split deve ter split_type válido ('percentage' ou 'fixed_amount')");
        }
        
        if (split.split_type === 'percentage') {
          if (!split.percentage || split.percentage < 0 || split.percentage > 100) {
            throw new Error("Split com tipo 'percentage' deve ter percentage entre 0 e 100");
          }
          if (!split.recipient_id) {
            throw new Error("Split deve ter recipient_id");
          }
        } else if (split.split_type === 'fixed_amount') {
          if (!split.amount || split.amount <= 0) {
            throw new Error("Split com tipo 'fixed_amount' deve ter amount positivo");
          }
          if (!split.recipient_id) {
            throw new Error("Split deve ter recipient_id");
          }
      ...
      // Calcular total dos splits
      const splitTotal = splits.reduce((sum, split) => {
        if (split.split_type === "percentage") {
          const calculatedAmount = Math.round((amount * split.percentage) / 100);
          return sum + calculatedAmount;
        } else {
          return sum + Math.round(split.amount);
        }
      }, 0);
      
      // Validar que soma dos splits = totalAmount (±0.01 tolerância)
      // Como estamos trabalhando em centavos, a tolerância é 1 centavo
      const tolerance = 1; // 1 centavo
      const difference = Math.abs(splitTotal - amount);
      
      if (difference > tolerance) {
        throw new Error(
          `Soma dos splits (${splitTotal / 100}) não corresponde ao valor total (${amount / 100}). ` +
          `Diferença: ${difference / 100}. Tolerância máxima: ${tolerance / 100}`
        );
      }
      
      logger.info('Splits validados', { 
