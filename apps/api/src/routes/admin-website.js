...
const getDataCollection = async (pageType) => {
  try {
    ...
    if (!Array.isArray(data)) {
      console.error(`❌ Erro: query não retornou array para ${pageType}`, typeof data);
      throw new Error(`Query não retornou array`);
    }

    // Converter campos JSONB de volta para objetos
    const result = data.map((item) => {
      try {
        // Parse seguro de JSONB
        let images = item.images || [];
        if (typeof item.images === 'string') {
          try {
            images = JSON.parse(item.images);
          } catch (e) {
            console.warn(`⚠️  Erro ao parse images para ${item.content_id}:`, e.message);
            images = [];
          }
        }
        ...
        if (typeof item.metadata === 'string') {
          try {
            ...
            console.warn(`⚠️  Erro ao parse metadata para ${item.content_id}:`, e.message);
            metadata = {};
          }
        }
        
        let seo_data = item.seo_data || {};
        if (typeof item.seo_data === 'string') {
          try {
            seo_data = JSON.parse(item.seo_data);
          } catch (e) {
            console.warn(`⚠️  Erro ao parse seo_data para ${item.content_id}:`, e.message);
            seo_data = {};
          }
        }
        ...
        // Retornar item básico se processamento falhar
        return {
          id: item.id,
          content_id: item.content_id || `item-${item.id}`,
    ...
    if (!Array.isArray(result)) {
      console.error(`❌ Erro: map não retornou array para ${pageType}`);
      return [];
    }
    ...
    switch (pageType) {
      case "hotels":
        fallbackData = hotelData || [];
        break;
      case "promotions":
        fallbackData = promotionsData || [];
        break;
      case "attractions":
        fallbackData = attractionsData || [];
        break;
      case "tickets":
        fallbackData = ticketsData || [];
        break;
      default:
        fallbackData = [];
    }
    // Garantir que fallback também é array
